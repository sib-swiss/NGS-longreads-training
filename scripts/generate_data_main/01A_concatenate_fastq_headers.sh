...
#SBATCH --job-name=concatenate_fastq
...
output_file=$(basename $input_file .fastq.gz).concatenated.fastq.gz

# replace all spaces with pipes in the fastq header. 
zcat "$input_dir"/"$input_file" | sed -n '1~4s/ /|/g;p' | gzip > "$output_dir"/"$output_file"
